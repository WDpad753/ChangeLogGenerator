            ...
            try
            {
                ...
                try
                {
                    ...
                    if (taskcol.IsFaulted)
                    {
                        ...
                        _logWriter.LogWrite($"Error in acquiring response from url {client.BaseAddress}: {taskcol.Exception.ToString()}", GetType().Name, UtilityClass.GetMethodName(), MessageLevels.Fatal);
                    }
                    else
                    {
